...
void createArucoMarkers()
{
    ...
    aruco::Dictionary dictionary = cv::aruco::generateCustomDictionary(number, dimension);
    Mat store = dictionary.bytesList;

    FileStorage fs("dic_save.yml", cv::FileStorage::WRITE);
...
int loadArucoMarkers(Mat& cameraMatrix, Mat& distanceCoefficients, float arucoSquareDimension)
{
  ...
  fsr["ByteList"] >> bits;
  fsr.release();
  cv::aruco::Dictionary markerDictionary = cv::aruco::Dictionary(bits, mSize, mCBits);
...
int main(int argc, char const *argv[])
{
  ...
  loadArucoMarkers(cameraMatrix, distanceCoefficients, arucoSquareDimensionMaior);
